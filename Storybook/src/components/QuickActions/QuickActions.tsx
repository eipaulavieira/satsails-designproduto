...
import withdrawIcon     from '../../assets/icons/phosphor/vender.svg';
import depositIcon      from '../../assets/icons/phosphor/currency-dollar.svg';

export type ActionItem = 'receber' | 'enviar' | 'converter' | 'vender' | 'depositar';
...
const ACTIONS: { id: ActionItem; label: string; icon: string }[] = [
  ...
  { id: 'vender',    label: 'Vender',    icon: withdrawIcon  },
  { id: 'depositar', label: 'Depositar', icon: depositIcon   },
];

export function QuickActions({ onChange }: QuickActionsProps) {
  ...
  return (
    <div style={{
      ...
      {ACTIONS.map(({ id, label, icon }) => {
        const isDepositar = id === 'depositar';
        return (
          <button
            key={id}
            ...
            style={{
              ...
              background: isDepositar ? '#00C853' : 'var(--dark-new, #080808)',
              cursor: 'pointer',
              padding: 0,
              flexShrink: 0,
            }}
          >
            <img
              ...
              height={18}
              alt={label}
              style={{ display: 'block', filter: isDepositar ? 'brightness(0)' : undefined }}
            />
            <span style={{
              color: isDepositar ? 'var(--dark-new, #080808)' : '#727272',
              textAlign: 'center',
